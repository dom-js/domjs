    ...
    var ws=function(opts){
                    ...
                    if( params.length){
                        $.each(params,function(i,p){
                            ...
                            if(typeof p.value!="undefined"){
                                data +='<web:'+ p.name+'>'+ p.value+'</web:'+ p.name+'>';
                            }
                        })
                    }else{
                        data += params.value ;
        ...
        var factory = function(doc){
            ...
            $.each(_schema.childNodes,function(i,el){
                ...
                if(el.nodeType==1){
                    var name = el.getAttribute("name");
                    var paras = el.getElementsByTagName("element");
                    paras=paras.length==0?el.getElementsByTagName("xsd:element"):paras;
                    if(paras.length==0){
                        parameters[name]=name;
                    }else{
                        parameters[name] = $.map(paras,function(el){
        ...
        _wslist[wsdl] =  xhr({onlyie:opts.onlyie,url:wsdl,dataType:"text"}).pipe(function(strXml){
            var doc = $.parseXML(strXml);
            return factory(doc)
        },function(e){
            new Error(e)
