/**
 * 2014-6-12 优化ws方法，支持schema 从独立文件导入，支持传递authorization等headers信息进行身份验证
 **/
define( function() {
    var  __rkeys={
        //"a0z":"A","a1z":"B","a2z":"C","a3z":"D","a4z":"E","a5z":"F","a6z":"G","a7z":"H","a8z":"I","a9z":"J",//数字
        "a.z":"K","a,z":"L","a?z":"M","a;z":"N","a:z":"O","a'z":"P","a\"z":"Q","a\\z":"R",//标点
        "a+z":"S","a-z":"T","a*z":"U","a\/z":"V","a%z":"W",//算数运算
        "a>z":"X","a<z":"Y","a=z":"Z","a!z":"A","a|z":"B","a&z":"C","a^z":"D",//逻辑运算
        "a(z":"E","a)z":"F","a[z":"G","a]z":"H","a{z":"I","a}z":"J",//括号运算
        "a`z":"1","a~z":"2","a@z":"3","a#z":"4","a$z":"5","a_z":"6","a z":"7"
    ...
    var ws=function(opts){
        ...
        if(_wslist[wsdl])return _wslist[wsdl]
        var soapFactory=function(url,soapaction,namesacpe,name,params,resname){
            ...
            var action = function(){
                $.each(arguments,function(i,v){
                    ...
                    if(params && params[i]) params[i].value = v;
                })
        ...
        var factory = function(doc){
            ...
            var getParams = function(el,_schema){
                ...
                var name = el.getAttribute("name");
                var paras =  $(el).find("[name]").get()  //el.getElementsByTagName("element");
            ...
            if(_schema.childNodes.length==1&&/import$/.test(_schema.childNodes[0].tagName)){
                ...
                var _p= $.extend({},opts,{
                    async:false,
                    url:_schema.childNodes[0].getAttribute("schemaLocation")
                })
    ...
    var getremotemodule=function(url,options,cb){
        ...
        return $.ajax({
            dataType: "script",
            url: url,
            data: $.extend( {},cbinfo,options)
        }).pipe(function (script) {
                var def = new $.Deferred();
                require([key],function(data){
                    def.resolve(data)
                });
                setTimeout(function(){def.reject("数据加载超时")},15000)
                return def;
            });
    }
    var util = {
        ...
        ws:ws,
        /**
         * 将对象混入另一个对象 obj1，对象一，obj2，对象2，flag：是否修改对象1 ，ture
         * 修改，false，不修改default:false
         */
        ...
        xmlToString:function(xmlData){ // this functions waits jQuery XML
                    ...
                    case  "[object Function]":
                        ...
                        if(name){
                            if(window[name])
                                ...
                                return "Math."+name
                            else if( ! /native code/.test(obj.toString()))
                                return  String(obj);
                            else
        ...
        strToKey:function(str){
            return typeof str!="string"?str:encodeURIComponent(str.replace(/[\`\~\!\@\#\$\%\^\&\*\(\)\_\+\-\=\{\}\[\]:\"\|\;'\\\<\>\?,\.\/\s]/g,
                __getKeyCode)).replace(/\%/g,"")
        },
        /**
         * Domino URL命令对象，
         */
        ...
        isIE6:($.browser.msie && parseInt($.browser.version) < 0x7)||document.compatMode=="BackCompat",
        ...
        isNotesBox:function(){
            ...
            return false
            // "Mozilla/4.0 (compatible; Lotus-Notes/6.0; Windows-NT)".match(/lotus/gi)
        },
        uid:uid,
        /**
         * 数字/字符串数组去重
        ...
        unique:function(arr){
            ...
            for(var i = arr.length-1;i>=0;i--)
                o[arr[i]]?(arr.splice(i,1)):o[arr[i]]=0x1 ;
            return arr;
        },
        ...
        error:function(err){
            ...
            if(err.requireModules){
                ...
                $.each(err.requireModules,function(i,errm){
                    if(typeof console!= "undefined"&&  typeof console.warn!= "undefined"){
                        throw err
                        // console.warn("模块"+errm+"错误",":",err.message)
                        //  console.warn(err.stack)
                    }else{
                        require(["widget/jbox"],function(jbox){
                            jbox(err.stack.replace(/\n/,"<br>"),{
                                title:"模块"+errm+":"+err.message,
                                width:800
                            })
                        })
                    }
        ...
        getBytesCount:function(str)
        {
            ...
            {
                //如果是单字节，则将双字节替换冲空值后，再取一次双字节的长度（既剩余的为双字节值计算两次）
        ...
        winclose : function(refresh) {
            ...
            if(typeof window.postMessage !="undefined"){
                ...
                if(window.parent){
                    try{ window.parent.postMessage("childwindowclose","*")}catch(e){}
            ...
            //如果需要刷新页面,则执行页面刷新尝试，同时发布一个刷新通知
            if (refresh) {
                try {
                    if (window.opener) {
                        if (window.opener.refreshPortletList
                    ...
                    try {
                        document.domain = "xxx.com.cn"; // 如果父窗口刷新错误，尝试修改domain再进行刷新
