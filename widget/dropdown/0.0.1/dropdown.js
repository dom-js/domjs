	...
	DropDown.panel =function(ops){
		...
		this.contentPanel=$("div[data-type=dropdown][jId=" +this.jId+ "]");
		 ...
		 if(this.isCreated ||!this.contentPanel.is("[jId=dpnames]")) {
			 this.contentPanel = $("<div  tabindex=1 data-type='dropdown' style='z-index:9999;display:none;background-color:#FFF;border:\" solid  1px #ccc\"' jId='"+this.jId+"'>").css({
				 position:"absolute"
			 });
             this.contentPanel.on("blur",function(){
                 me.hide();
             })
		 ...
		 if(!this.listPanel.is()){
			 this.listPanel = $("<ul class='dropdownul' style='margin:0px;padding: 0px'>") ;
			 this.contentPanel.append(this.listPanel);
		 }
		 this.addEntry=function(el){
			 var entry = $("<li class='drowdownli' style='list-style:none'>");
