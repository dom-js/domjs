...
define(["base/objectj","widget/jbox","i18n!nls/system"], function (obj,jbox,i18n) {
    ...
    levelselect.fn.extend({
              ...
              //  _self.locked = true
               // _self._currentResult.then(function(){
                    ...
                    _self._getResult($(_this)).then(function(data){
                        ...
                        var selectable = _self.options.selectcate ||!expendable || data.length==0
                        //不能选择时，禁用选择按钮
                       // if(!selectable){
                       ...
                       if(expendable && (!isSelect)){
                        ...
                        if(data.length>0 && expendable){
                           ...
                           // itemextDom.text(">")
                                ...
                                if(itemextDom.html()==""){
                                    if(_self.options.multi){
                                         var checkbox = $("<input  style='width: 16px;height: 16px;line-height: 25px;' type='button' title='"+i18n["Invert Selection"]+"' name='levellistselectallcheckbox' style='margin-top: 3px'/>  ")
        ...
        _initSelected:function(){
            ...
            var item,obj,items = this.options.valueitems
                ...
                for(var i in items){
                    ...
                    if(item)
                    this.panel.select.append(this._createSelectItem(item))
                }
        ...
        _refreshItemCss:function(itemDom){
            ...
            //更新itemDom自身样式
            itemDom.css({
                "background-color":itemDom.data("focus")&&itemDom.data("select")?"#99eeff":itemDom.data("focus")?"#efefef":itemDom.data("select")?"#cceeff":""
            })
           //更新Dom上级样式

            var levelkey = itemDom.data("levelkey")||0
                if (levelkey>0){
                 //   this.focusItemQueue[itemDom.data("levelkey")-1]
                }
         //  console.log()
           // console.log(itemDom.data("columnkey"))
        },
        ...
        _getItemDom:function(item,levelkey,columnkey){
            ...
            var li = $("<li class=\"levellistitem\" style=\"padding:0px 5px;cursor: pointer\"></li>");
            var label = $("<span class='levellistitemlabel'></span>")
            label.text(  item[this.options.label]||item[0]||(item._columns&&item[item._columns[0]])||"undefined label")
            var ico = $("<span class='levellistitemico'></span>"),ext = $("<span class='levellistitemext' style='height: 25px;line-height: 25px;'></span>")
            li.append(ico)
            if(this.options.multi){
            //  var checkbox = $("<input type='checkbox' name='levellistitemcheckbox'/>")
              //  li.append(checkbox)
            }
            li.append(label)
            li.append(ext)
        ...
        _showLevel:function(flag){
            ...
            if(this.focusLevel-this.firstLevel>=interval){  //当前位置与firstlevel 相差 showcols，则 更新firstlevel，
                if(flag&&this.focusLevel>0) this.focusLevel--
        ...
        //创建一个选择后的节点
        _createSelectItem:function(itemDom){

            var checkbox = $("<input type='checkbox' checked name='levellistitemcheckbox'/>")
            var item ,_dom
            var li
            ...
            if(this.options.multi){
                ...
                if(itemDom.data("select")){
                    if(selectDom.length==0)
                        this.panel.select.append(this._createSelectItem(itemDom))
                }else{
                    selectDom.remove()
                }
                this.panel.select.find(">li[data-itemkey]").each(function(i,selectDom){
