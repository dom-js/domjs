...
define(["base/objectj","widget/jbox","i18n!nls/system","plugin/util"], function (obj,jbox,i18n,util) {
    ...
    levelselect.fn.extend({
        ...
        showDialog:function(){
            ...
            if(this.options.multi)
                buttons[i18n["Empty"]] ="empty"
            else
                buttons[i18n["Clear"]] ="empty"
        ...
        _refreshPath:function(){
            ...
            if(itemDom && itemDom.data("item") && itemDom.data("item")[this.options.label]){
                ...
                var store = itemDom.data("store")||this.store
                var newcolkey = itemDom.data("item")[store.idProperty]
                if(node.length==0){
        ...
        _getColumnDom:function(data,levelkey,columnkey){
            ...
            if(_hasdom){
                var store = columndom.data("store")||this.store
                for(var i in columndom.data("data")){
                    if(columndom.data("data")[i][store.idProperty]!=data[i][store.idProperty]){
                        _hasdom = false
                    }
                }
            }
        ...
        _getItemDom:function(item,levelkey,columnkey){
            ...
            //计算数据源
            var preleveldom = this.levelQueue[levelkey-1]
            var store = item.__store__

            if(!store){
                if(preleveldom){
                    var parentdom =  preleveldom.find(">li[data-itemkey='"+columnkey+"']")
                    if(parentdom.length>0){
                        if(parentdom.data)
                            if(parentdom("store"))
                                store = parentdom("store")
                            else if(parentdom.data("item")&&parentdom.data("item").__store__)
                                store = parentdom.data("item").__store__
                    }

                }
                item.__store__   =   store = store || this.store

            }

            var itemDom =  this._createItemDom(item)
            itemDom.data("store",store)
        ...
        _getSelectDom:function(itemDom){
            ...
            var store =itemDom.data && itemDom.data("store")||this.store
            li.data("store",store)
            if(item[store.idProperty]){
                li.attr("data-itemkey",item[store.idProperty])
            }else{
        ...
        _getResult:function(itemDom){
            ...
            var resultkey = itemDom.data("item")[store.idProperty];
            // if(_self.options.cacheresult)
            var result =_self.options.cacheresult ? this.resultQueue[resultkey]:false;
            if(!result){
                //初始化时载入数据，对数据进行处理
                result = this.resultQueue[resultkey] =   store.getChildren(itemDom.data("item")).map(function(item){
        ...
        _expendItemDom:function(itemDom,levelkey){
            ...
            this._expendResult(result ,levelkey,itemDom.data("item")[store.idProperty]);
        },
        //被选择执行选中操作
        _onselect:function(itemDom){
            ...
            if(this.options.multi){
                ...
                var itemkey = item[store.idProperty]
        ...
        matchSelect:function(itemOrg,itemSel){
            var  store1 = itemOrg.data && itemOrg.data("store")|| this.store
            var  store2 = itemSel.data && itemSel.data("store")|| this.store
            if(itemSel[store1.idProperty]){
                return itemSel[store1.idProperty] == itemOrg[store2.idProperty]
